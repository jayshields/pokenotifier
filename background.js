...
function getLocation() {
  if (navigator.geolocation) {
    ...
    $('#status').html("Location unavailable.");
  }
}
...
{   
  $.getJSON('https://pokevision.com/map/data/' + lat + '/' + lng, {}, function (data) {
    ...
    $.each(data.pokemon, function () {
      ...
      {
        freshlycatchable.push(this.pokemonId);
      }        
    });
    
    console.log('prev:');
    console.log(previouslycatchable);
    
    console.log('fresh:');
    console.log(freshlycatchable);
    
    //Get the ones to alert about
    var catchable_diff = freshlycatchable.filter(function(x) { return previouslycatchable.indexOf(x) < 0 })
    console.log('diff;');
    console.log(catchable_diff);

    $.each(catchable_diff, function(){
      if(first === false)
        var notification = new Notification(pokedex[this], {
          icon: 'https://ugc.pokevision.com/images/pokemon/'+this+'.png',
          body: "A wild "+pokedex[this]+" has appeared!",
          title: pokedex[this]
      });
    });

    //Update badge
    chrome.browserAction.setBadgeText({text:freshlycatchable.length+''});
